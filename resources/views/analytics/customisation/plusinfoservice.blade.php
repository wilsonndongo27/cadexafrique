...
<div class="container-fluid">
    ...
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item active">Gestion des détails sur le service <span style="color:blue">{{$service->title}}</span></li>
    </ol>
    <table id="articles-table" class="table shadow-card">
        <thead style="margin-left:5px">
            <tr style="font-size: 14px;">
                ...
                <a href="#" id="AddDetailService" data-toggle="tooltip" title="Ajouter un détail">
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-content">
                              <a href="Javascript:void()" id="updatedetailservice"
                                data-detailid="{{ $item->id }}"
                                data-title="{{ $item->title }}"
                                data-description="{{ $item->description }}"
                                data-photo="{{ asset('storage/'.$item->cover) }}"
                              >Modifier</a>
                              <a href="JavaScript:void()" id="deletedetailservice"
        ...
        @else
            <tr >
                <td colspan="7" class="text-primary text-center" >
                    <p style="font-size: 14px; margin-top:50px; color:#888">Pas de détail enregistrer pour ce service!</p>
                </td>
            </tr>
        @endif
        </tbody>
    ...
    <div id="AddServiceDetailModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header row">
                    <img style="height:50px; border-radius:5px; margin-left:10px" src="{{asset('images/logo-cadex.jpg')}}"/>
                      <h4 class="modal-title" style="margin-left: 11px;font-size:18px; font-weight:bold">{{ __('Ajouter détail sur ce service') }}</h4>
                    <button style="margin-right: 10px" type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <form id="createform" lpformnum="1"  method="post" enctype="multipart/form-data" onsubmit="postSubmission()" action="{{ route ('create-detail-article')}}">
    ...
    <div id="UpdateDetailServiceModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header row">
                ...
                <form id="updateform" lpformnum="1"  method="post" enctype="multipart/form-data" onsubmit="postSubmissionUpdate()" action="{{ route ('update-detail-article')}}" >
                    <div class="modal-body" style="font-size:14px">
                        @csrf
                        <input type="hidden" name="detail_service_id" id="detail_id"/>
                        <div class="row" style="margin-top: -10px;">
                            <div class="col-lg-12 col-md-12 col-xs-12">
                                <p style="color: #888">{{  __('Mise à jour du titre') }}</p>
                                <div class="form-group label-floating" style="width: 100%;">
                                    <input class="form-control" id="titledetailservice" name="title" style="border-radius:5px"  placeholder="title of the message" />
                                    <hr>
                                </div>
                            </div>
                        </div>
    ...
    <div id="DeleteDetailServiceModal" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <form id="delete_form" lpformnum="1"  method="post" enctype="multipart/form-data" action="{{ route ('delete-detail-article')}}">
                    <div class="modal-body" style="font-size:14px">
                        @csrf
                        <input type="hidden" name="currentservicedetail" id="iddetail" />
                        <div class="row" style="margin-top: -10px;">
                            <div class="col-lg-12 col-md-12 col-xs-12">

...
<div class="container-fluid">
    ...
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item active">Gestion des détails sur la bannière <span style="color:blue">{{$banniere->title}}</span></li>
    </ol>
    <table id="articles-table" class="table shadow-card">
        <thead style="margin-left:5px">
            <tr style="font-size: 14px;">
                ...
                <a href="#" id="AddDetailBanniere" data-toggle="tooltip" title="Ajouter un détail">
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-content">
                              <a href="Javascript:void()" id="updatedetailbanniere"
                                data-detailid="{{ $item->id }}"
                                data-title="{{ $item->title }}"
                                data-description="{{ $item->description }}"
                                data-photo="{{ asset('storage/'.$item->cover) }}"
                              >Modifier</a>
                              <a href="JavaScript:void()" id="deletedetailbanniere"
        ...
        @else
            <tr >
                <td colspan="7" class="text-primary text-center" >
                    <p style="font-size: 14px; margin-top:50px; color:#888">Pas de détail enregistrer pour cette bannière!</p>
                </td>
            </tr>
        @endif
        </tbody>
    ...
    <!-- modale de creation du détail dune banniere -->
    <div id="AddBanniereDetailModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header row">
                    <img style="height:50px; border-radius:5px; margin-left:10px" src="{{asset('images/logo-cadex.jpg')}}"/>
                      <h4 class="modal-title" style="margin-left: 11px;font-size:18px; font-weight:bold">{{ __('Ajouter des détails pour cette bannière') }}</h4>
                    <button style="margin-right: 10px" type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <form id="" lpformnum="1"  method="post" enctype="multipart/form-data" onsubmit="postSubmission()" action="{{ route ('create-detail-banniere')}}">
                    <div class="modal-body" style="font-size:14px">
                        @csrf
                        <input type="hidden" name="creator" value="{{ auth()->user()->id }}"/>
                        <input type="hidden" name="banniere_detail_id" value="{{ $banniere->id }}"/>
    ...
    <div id="UpdateDetailBanniereModal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header row">
                ...
                <form id="updateform" lpformnum="1"  method="post" enctype="multipart/form-data" onsubmit="postSubmissionUpdate()" action="{{ route ('update-detail-banniere')}}" >
                    <div class="modal-body" style="font-size:14px">
                        @csrf
                        <input type="hidden" name="detail_banniere_id" id="detail_id"/>
                        <div class="row" style="margin-top: -10px;">
                            <div class="col-lg-12 col-md-12 col-xs-12">
                                <p style="color: #888">{{  __('Mise à jour du titre') }}</p>
                                <div class="form-group label-floating" style="width: 100%;">
                                    <input class="form-control" id="titledetailbanniere" name="title" style="border-radius:5px"  placeholder="title of the message" />
                                    <hr>
                                </div>
                            </div>
                        </div>
    ...
    <!-- modale pour supprimer le detail dune banniere -->
    <div id="DeleteDetailBanniereModal" class="modal fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <form id="delete_form" lpformnum="1"  method="post" enctype="multipart/form-data" action="{{ route ('delete-detail-banniere')}}">
                    <div class="modal-body" style="font-size:14px">
                        @csrf
                        <input type="hidden" name="currentbannieredetail" id="iddetail" />
                        <div class="row" style="margin-top: -10px;">
                            <div class="col-lg-12 col-md-12 col-xs-12">
